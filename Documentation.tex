...
on the same machine the \cleanstyle{localhost=127.0.0.1} IP address is
...
\cleanstyle{CMOD\_PORT=9002} on the local machine we need the only to
know the couple \cleanstyle{localhost:CMOD\_PORT} or explicitly
...
\begin{figure}
  \includegraphics[width=\linewidth]{/opt/anpan/doc/_images/state_machine.doc.png}
...
the \cleanstyle{cmd\_acqpc.py} file where I added many explanatory comments to the
code.
...
% pyrame/launcher/services.txt
% To add default module ports edit the file pyrame/ports.txt
\section{Anpan}
\begin{itemize}
